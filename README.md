# DAG_ModelComparison

This program uses Herbie & xESMF to analyze temperature data from the HRRR & RTMA run at NCEP. MatPlotLib & Cartopy are then used to plot the data in a map.

TODO: Last Updated - 11.16.25 0900Z

Short term (by 11.25)
Make title work plotting.py in the same manner that it works in the Jupyter notebook (COMPLETED!)
Develop a method that can remove GRIB2 files after downloading to reduce clutter on the end-user side (COMPLETED!)

Medium term (by 12.10)
Develop tests that makes sure that calculations & plotting are correct
Develop method for testing NAM3K against RTMA

Long term (12.31)
Develop method for testing HRRR against NAM3K
Develop method for testing HRRR/NAM3K against URMA
